...
const std::vector<uint8_t> Address::get_octets() {
	return octets;
}
...
bool operator==(Address &lhs, Address &rhs) {
	...
	if (leftVector[0] == rightVector[0] && leftVector[1]==rightVector[1] && leftVector[2]==rightVector[2] && leftVector[3] == rightVector[3]) {
		return true;
	} else {
		return false;
	}
