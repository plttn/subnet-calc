...
void Subnet::load_subnet(std::string input) {
  ...
  maskLength = atoi(tok);
  this->inputAddress.set_address(
      inputAddress); // now this->inputaddress contains address in vector
  // build mask address
...
std::vector<uint8_t> Subnet::convert_cidr(int cidr) {
  ...
  for (size_t i = 0; i < 4; i++) {
    if (cidr >= 8) {
      ...
      partial =
          partial << multiplier; // account for subtracting effectively giving
                                 // you lower end of octet, so shift over by
                                 // number its short by to get it on MSB
...
void Subnet::print_subnet() {
  std::cout << "Input Address: " << this->inputAddress.print_address()
            << std::endl;
  std::cout << "Classful Net Address: " << this->classfulAddress.print_address()
            << std::endl;
  std::cout << "Classful Broadcast: " << this->classfulBC.print_address()
            << std::endl;
  std::cout << "Subnet Address: " << this->subnetAddress.print_address()
            << std::endl;
  std::cout << "Subnet Mask: " << this->maskAddress.print_address()
            << std::endl;
  std::cout << "Subnet Broadcast: " << this->subnetBC.print_address()
            << std::endl;
  ...
  std::cout << "Number of usable hosts per subnet: " << this->numHosts
            << std::endl;
  std::cout << "Number of usable subnets for class: " << this->numSubnets
            << std::endl;
  std::cout << "Host range: " << this->lowAddr.print_address() << "-"
            << this->highAddr.print_address() << std::endl;
